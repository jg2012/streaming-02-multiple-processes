...
INPUT_FILE_NAME = "atp_tennis.csv"

# Define program functions (bits of reusable code)


def prepare_message_from_row(row):
    print("Number of elements in row:", len(row))

    """Prepare a binary message from a given row."""
    Tournament, Year, Month, Day,Series,Court,Surface,Round,Best_of, Player_1, Player_2, Winner,Rank_1,Rank_2,Pts_1,Pts_2,Odd_1, Odd_1, Score= row
    # use an fstring to create a message from our data
    # notice the f before the opening quote for our string?
    fstring_message = f"[{Tournament},{Year}, {Month}, {Day},{Series}, {Court}, {Surface}, {Round}, {Best_of}, {Player_1}, {Player_2}, {Winner}, {Rank_1}, {Rank_2}, {Pts_1}, {Pts_2}, {Odd_1}, {Odd_1}, {Score}]"
...
def stream_row(input_file_name, address_tuple):
    ...
    with open("out9.txt", "w") as output_file:
        logging.info("Opened output file for writing: out9.txt.")
        ...
        with open(input_file_name, "r") as input_file:
            ...
            for row in rows:
                ...
                # Stream the message over the socket connection
